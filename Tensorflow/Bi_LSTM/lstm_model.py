...
##    word_lstm_output = tf.nn.dropout(word_lstm_output, keep_prob=keep_prob, name='word_lstm_output')
    stack_drop = [];
...
def build_bidirectional_lstm(num_cells, num_layers, batch_size, keep_prob):

    print('build bi-directional lstm');
    fw_cell = tf.nn.rnn_cell.LSTMCell(num_units=num_cells, state_is_tuple=True);
    bw_cell = tf.nn.rnn_cell.LSTMCell(num_units=num_cells, state_is_tuple=True);
...
##    seq_output = tf.concat(lstm_out, 1) # important
    input_x = tf.reshape(lstm_out, [-1, in_size]);
